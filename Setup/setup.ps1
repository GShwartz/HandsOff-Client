...
$registry_all_users = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Scripts\Startup\1'
$registry_name = 'HandsOff'
$registry_value = 'c:\HandsOff\client.bat'

Write-Host "Setting up persistence..."
Set-ItemProperty -Path $registry_all_users -Name $registry_name -Value $registry_value -Force
#New-Item -Path "$registry_path" -Force | New-ItemProperty -Name "$registry_name" -Value "$registry_value" -PropertyType "String"
Write-Host "Done."
